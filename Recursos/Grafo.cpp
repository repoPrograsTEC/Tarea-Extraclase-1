...
class Dijkstra{
public:
    static int algoritmo(int, int, const Graph&);
};

int Dijkstra :: algoritmo(const int begin, const int end, const Graph& graph){
...
void Programa :: Dijkstra(const Graph& graph){
    ...
    cin >> fin;
    int n = Dijkstra::algoritmo(comienzo, fin, graph);  //Accediendo al algoritmo de Dijkstra
    if (n >= 0){
